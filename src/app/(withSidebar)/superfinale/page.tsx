...
export default function SuperfinaleHome() {
  type TrimestryKey = "T1" | "T2" | "T3";

  const { data, isLoading } = useTournamentDataByCategory("superfinale");
    ...
    const actions: ActionDefinition<TournamentRow>[] = [
      {
        ...
        onClick: () => window.open(`/superfinale/${item.id}`, "_self"),
      },
    ];
  ...
  const handleViewTournament = (id: string) => {
    window.open(`/superfinale/${id}`, "_self");
  };
      ...
      <div className="hidden md:flex flex-col gap-3">
        ...
        <div className="w-full overflow-x-auto">
          <GenericTable<TournamentRow>
            ...
            enableRowClick
            getDetailUrl={(id) => `/superfinale/${id}`}
          />
        </div>
      </div>
